...
bool SceneTestBloom::Initialize()
{
	//ライト初期化
	Light::Init();
	...
	CreateConstantBuffer(bloomCB.GetAddressOf(), sizeof(ConstantBufferForBloom));
...
//	bokehEx->Create(L"Shaders/bokeh_vs", L"Shaders/bokeh_ps");
	bokehEx->Create(L"Shaders/gaussianFilter_vs", L"Shaders/gaussianFilter_ps");
...
//
//---------------------------------------------------
void SceneTestBloom::imGuiUpdate()
{
	...
	// 2. Show a simple window that we create ourselves. We use a Begin/End pair to created a named window.
	{
		...
		ImGui::SliderFloat(u8"標準偏差", &Sigma, 0.0f, 2.0f);
	...
	if (texture_window)
	{
		ImGui::Begin(u8"テクスチャ", &show_another_window);   
		{
			...
			ImGui::Separator();
			ImGui::Text("bright_texture");
			ImGui::Image(bright_texture->GetShaderResourceView(), { w, h });
			ImGui::Separator();
			ImGui::Text("bloom_texture");
			ImGui::Image(bloom_texture->GetShaderResourceView(), { w, h });
		}
		ImGui::End();
	}
}

void SceneTestBloom::Update()
{
	//ライト・カメラ操作設定
	Scene::Operate();
...
//
//---------------------------------------------------
void SceneTestBloom::RenderScreen()
{
...
bool SceneTestBloom::GaussianFilter(DirectX::XMFLOAT4* array, int karnel_size, float sigma)
{
	float sum = 0.0f;
	int id = 0;
...
void SceneTestBloom::RenderBloom()
{
	ID3D11DeviceContext* device_context = pSystem->GetDeviceContext();
	{
		// コンスタントバッファ設定
	...
	{//高輝度抽出
		...
		//コンスタントバッファ更新
		device_context->UpdateSubresource(bloomCB.Get(), 0, NULL, &cb, 0, 0);
		device_context->VSSetConstantBuffers(4, 1, bloomCB.GetAddressOf());
		device_context->PSSetConstantBuffers(4, 1, bloomCB.GetAddressOf());
	...
	{//ぼけ画像作成
		...
		GaussianFilter(weight, KARNEL_SIZE-1, Sigma);
		...
		cb.texcel.x = 8.0f / screen_texture->GetWidth();
		cb.texcel.y = 8.0f / screen_texture->GetHeight();
		//コンスタントバッファ更新
